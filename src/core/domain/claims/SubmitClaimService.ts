...
export class SubmitClaimService {
  ...
  async execute(input: ClaimSubmissionInput): Promise<{
    ...
    if (expectedDetailType !== input.detailType) {
      return {
        claimId: null,
        errorCode: "DETAIL_TYPE_MISMATCH",
        errorMessage: "Claim detail type does not match selected payment mode.",
      };
    }
    ...
    if (input.detailType === "expense" && input.expense) {
      payload.expense = {
        ...
        total_amount: input.expense.totalAmount,
        currency_code: input.expense.currencyCode,
        vendor_name: input.expense.vendorName,
        receipt_file_hash: input.expense.receiptFileHash,
    ...
    if (input.detailType === "advance" && input.advance) {
      payload.advance = {
        ...
        supporting_document_hash: input.advance.supportingDocumentHash,
