-- Migration: Enrich vw_enterprise_claims_dashboard to include submitter label,
-- category name, purpose, and file paths so the My Submissions table can be
-- fully rendered from a single query without a secondary getClaimListDetails fetch.
...
select
  ...
  coalesce(
    ...
    case
      ...
      when c.status in ('Rejected - Resubmission Not Allowed'::public.claim_status, 'Rejected - Resubmission Allowed'::public.claim_status) and c.assigned_l2_approver_id is null then c.updated_at
      else null
    end
  ) as hod_action_date,
  coalesce(
    ...
    case
      ...
      when c.status in ('Rejected - Resubmission Not Allowed'::public.claim_status, 'Rejected - Resubmission Allowed'::public.claim_status) and c.assigned_l2_approver_id is not null then c.updated_at
      else null
    end
  ...
  -- Enriched columns for single-pass My Submissions rendering
  case
