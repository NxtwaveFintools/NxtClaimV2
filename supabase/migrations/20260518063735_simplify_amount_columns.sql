-- Migration: simplify_amount_columns
-- Drops `approved_amount` from expense_details and advance_details.
-- Renames `requested_total_amount` to `total_amount` in both tables.
-- Recreates all dependent views and RPCs.

-- ─────────────────────────────────────────────────────────────
-- Step 1: Drop views that reference the affected columns
-- ─────────────────────────────────────────────────────────────
DROP VIEW IF EXISTS public.vw_admin_claims_dashboard;
DROP VIEW IF EXISTS public.vw_enterprise_claims_dashboard;

-- ─────────────────────────────────────────────────────────────
-- Step 2: Drop approved_amount columns
-- ─────────────────────────────────────────────────────────────
ALTER TABLE public.expense_details DROP COLUMN approved_amount;
ALTER TABLE public.advance_details DROP COLUMN approved_amount;

-- ─────────────────────────────────────────────────────────────
-- Step 3: Rename requested_total_amount → total_amount
-- ─────────────────────────────────────────────────────────────
ALTER TABLE public.expense_details RENAME COLUMN requested_total_amount TO total_amount;
ALTER TABLE public.advance_details RENAME COLUMN requested_total_amount TO total_amount;

-- ─────────────────────────────────────────────────────────────
-- Step 4: Recreate RPCs with updated column references
-- ─────────────────────────────────────────────────────────────

-- 4a. create_claim_with_detail
...
declare
  ...
  v_expense_total_amount numeric;
  v_advance_total_amount numeric;
...
begin
  ...
  if v_detail_type = 'expense' then
    ...
    v_expense_total_amount := round(v_basic_amount + v_cgst_amount + v_sgst_amount + v_igst_amount, 2);
  ...
  if v_detail_type = 'advance' then
    v_advance_total_amount := (p_payload->'advance'->>'total_amount')::numeric;
    v_advance_budget_month := (p_payload->'advance'->>'budget_month')::integer;
    v_advance_budget_year := (p_payload->'advance'->>'budget_year')::integer;

    if v_advance_total_amount is null then
      raise exception 'Advance total_amount is required';
    end if;
    if v_advance_total_amount <= 0 then
      raise exception 'Advance total_amount must be greater than zero';
    end if;
...
-- 4b. update_claim_by_finance (simple/legacy overload)
...
-- 4c. update_claim_by_finance (full overload with audit log)
...
-- 4e. process_l2_mark_paid_transition
...
-- 4f. refresh_claim_analytics_snapshot
...
begin
  ...
  select
    ...
    coalesce(ed.total_amount, ad.total_amount, 0)::numeric(14,2) as total_amount
  into v_claim
  from public.claims c
...
-- ─────────────────────────────────────────────────────────────
-- Step 5: Recreate views with updated column references
-- ─────────────────────────────────────────────────────────────
