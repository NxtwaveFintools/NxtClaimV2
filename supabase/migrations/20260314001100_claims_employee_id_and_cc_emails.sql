...
begin
  ...
  values (
    ...
    coalesce(nullif(p_payload->>'employee_id', ''), ''),
    nullif(p_payload->>'cc_emails', ''),
  ...
  if v_detail_type = 'expense' then
    ...
    values (
      ...
      (p_payload->'expense'->>'product_id')::uuid,
      ...
      (p_payload->'expense'->>'receipt_file_path'),
...
-- 1) alter table public.claims drop column if exists employee_id;
-- 2) alter table public.claims drop column if exists cc_emails;
