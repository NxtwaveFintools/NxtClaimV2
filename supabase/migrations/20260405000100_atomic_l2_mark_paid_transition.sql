-- Atomic single-claim mark-paid transition.
--
-- Why:
-- 1) Prevent read-modify-write races in application-layer wallet updates.
-- 2) Guarantee claim status transition, wallet increment, and audit logging
...
-- 1) drop function if exists public.process_l2_mark_paid_transition(text, uuid);
