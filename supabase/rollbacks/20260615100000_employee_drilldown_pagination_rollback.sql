-- Rollback: restore get_employee_claim_master to pre-pagination signature.

BEGIN;

DROP FUNCTION IF EXISTS public.get_employee_claim_master(UUID[], DATE, DATE, TEXT, UUID, UUID, TEXT, INT, INT);
...
AS $$
  ...
  SELECT employee_id, employee_name, total_amount::NUMERIC, claim_count,
         expense_amount::NUMERIC, advance_amount::NUMERIC
  FROM grouped
  WHERE
